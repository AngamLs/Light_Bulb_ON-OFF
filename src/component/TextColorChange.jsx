...
export default function TextColorChange() {
  let [textChange, setTextChange] = useState(true);
  let [colorChange, setColorChange] = useState();

  function handleClick() {
    return setTextChange(!textChange);
  }

  function toggleColor() {
    return setColorChange(!colorChange);
  }

  return (
    <div>
      <h1>
        Turn on the bulb and change the text inside the button by clicking the
        ON/OFF Button!
      </h1>

      <svg
        className="bulb"
        style={{ background: colorChange ? "#FCE700" : "#BFDCE5" }}
        viewBox="-7 -5 40 40"
        width="30%"
        height="100%"
      ...
      <button
        style={{ background: colorChange ? "#D61C4E" : "#BFDCE5" }}
        onClick={() => {
          handleClick();
          toggleColor();
        }}
